...
| Ordering.SignalrHub | `src/python/ordering_signalrhub` (Python, Socket.IO); .NET reference retained at `src/Services/Ordering/Ordering.SignalrHub` | `ordering-signalrhub` 5112:80 | No OpenAPI; realtime contract frozen in `contracts/realtime/ordering-signalrhub.md` (+ `UpdatedOrderState.golden.json`): `/hub/notificationhub`, browser `access_token` query auth, per-user group, `UpdatedOrderState {orderId, status}`; consumes 6 order-status events (golden parity); `contracts/health/ordering-signalrhub.*`; aud `orders.signalrhub` | `src/python/ordering_signalrhub/tests` (unit + mixed-stack RabbitMQ realtime tests, `docker-compose-tests.yml`); `signalr.service.spec.ts` (Jest); `e2e/order-status-realtime.spec.ts` (Playwright journey) | `eshop/ordering.signalrhub:linux-latest` (.NET, rollback image) | `eshop/ordering.signalrhub:linux-latest` built from `src/python/ordering_signalrhub/Dockerfile` (pinned `python:3.12.11-slim-bookworm`, non-root) | replacement-implemented; cutover gated with the Socket.IO WebSPA (deploy/rollback both ends together — no SignalR/Socket.IO wire compatibility) | rebuild+redeploy .NET hub image from `Services/Ordering/Ordering.SignalrHub/Dockerfile` AND revert WebSPA to the `@microsoft/signalr` client in the same step |
